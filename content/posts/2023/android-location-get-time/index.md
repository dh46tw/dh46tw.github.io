...
cover:
  image: "attachments/cover-spacex-unsplash.png"
  relative: true
  alt: "Android 定位結果的 `Location.getTime()` 不一定是衛星時間"
  hidden: false
  hiddenInList: false
  hiddenInSingle: true
summary: 本文探討 Android 開發中，使用 Location API 取得的時間值，並非 GPS 衛星時間。內文提供實驗的過程與結果，並提供讀者可參考的解決方案。
---
![Android 定位結果的 `Location.getTime()` 不一定是衛星時間操作畫面1](attachments/cover-spacex-unsplash.png)
...
| 定位類型                                 | 實測結果                                                  |
| ------------------------------------ | ----------------------------------------------------- |
| LocationManager<br/>Network Provider | ![Android 定位結果的 `Location.getTime()` 不一定是衛星時間操作畫面2](attachments/android-location-manager-network.png) |
| LocationManager<br/>GPS Provider     | ![Android 定位結果的 `Location.getTime()` 不一定是衛星時間操作畫面3](attachments/android-location-manager-gps.png)     |
| Fused Location                       | ![Android 定位結果的 `Location.getTime()` 不一定是衛星時間操作畫面4](attachments/android-location-manager-fused.png)  |
